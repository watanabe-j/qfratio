...
- $\mathbf{P}$: matrix of correponding eigenvectors of $\mathbf{A} - q \mathbf{B}$
...
For the pesent purpose, a quadratic form is expressed as a weighted sum of
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
In these expressions, $\hat{s}$ is the unique root of
\begin{equation}
  ...
  \sum_{i=1}^{n}
    ...
    \right] ,
\end{equation}
and is found numerically.
It is easily confirmed that
$\qfrE \left( X_q \right) =  K_{X_q}' (0) = \sum_{i=1}^{n} \lambda_i (1 + \nu_i^2)$,
which is used to determine the above condition.
...
its error are subsequently dividied by `pi`.
...
These functions evaluate the saddlepiont approximations described above.
They first check whether $\sum_{i=1}^{n} \lambda_i (1 + \nu_i^2)$ is close to
`0` using `tol_zero`.
If not, they conduct numerical root-finding for the saddlepoint by
...
that the evaluation result can be instable.
