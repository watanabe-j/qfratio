...
the distribution-related functions only concern unit exponents.
...
\end{equation}
when denominator is defined, to simplify calculation.
...
package.
`dqfr_broda()` conducts numerical integration by the `C` function
`gsl_integration_qagi(..., epsabs, epsrel, limit)` from `GSL`, when
`use_cpp = TRUE` (default).
