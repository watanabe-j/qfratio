...
- $t_1, \dots, t_s$: parameters in generating functions; $s$ here denotes the number of arguments (typically $t = 2$ or $3$)
...
- $N_n \left( \cdot , \cdot \right)$: $n$-variate normal distribution
- $\bm{\mu}$: $n$-variate mean vector for $\m{x}$
- $\bm{\Sigma}$: $n \times n$ covariance matrix for $\m{x}$
...
- $\delta_i$: defined as $(\m{h}_i^T \bm{\mu})^2$
...
Seeing this requires some excursion into the maths involved.
...
When the range of $\m{B}$ is a subspace of that of $\m{D}$ (or vice versa),
...
$\m{A}_{\mathrm{new}} = \m{K}^T \m{A} \m{K}$, etc., where $\m{K}$ satisfies
$\m{K} \m{K}^T = \bm{\Sigma}$ [@MathaiProvost1992, chapter 3].
...
\tag{3}
\end{equation}
and for an positive-integral $p$ [@Watanabe2022cevo, eq. (B38)],
\begin{equation}
\begin{aligned}
  \E \! \left( \frac{(\m{x}^T \m{A} \m{x})^p}{(\m{x}^T \m{B} \m{x})^q (\m{x}^T \m{D} \m{x})^r} \right)
  = &
...
the coefficients of $t_1^{k_1} \dots t_s^{k_s}$ in the following expansions:
\begin{multline}
...
Actually, the above expressions for moments can be written in terms of $d$
...
(which can be hundreds of thousands for very large problems).
...
absolute values of the eigenvalues of $\m{A}$,
...
series expressions are evaluated by the "super-short" recusrive algorithm
...
and the initial conditions are $u_{0,i} = v_{0,i} = 0$ for all $i$.

The recursion for $a_{p,k} \left( \m{A}, \bm{\mu} \right)$ is
[@HillierEtAl2014, eqs. (31), (32), noting that $w$ should be index by $k$ as well]:
\begin{equation}
...
actual calculation is done by one of the following four "internal" functions,
for which `qfrm()` acts as a switcher:
...
They do the following sequence:
...
9. Return a structured object of the series and error bound
...
The other two functions have the same arguments, just dropping inapplicable
...
whereas `*_pj_*` and `*_pjk_*` are for terms in series truncated along
...
All these functions, excepting `d1_i()` and `arl()`, have two versions ending
...
algorithms reduce from full matrix arithmetics to element-wise ones of
...
1. Use `qfrm()` (or `qfmrm()`) with the desired arguments with a small `m`
   (perhaps the default `m = 100`), **assigning the result into an object**
    - This may yield a warning when clear non-convergence is detected
...
#####
## V: Difficult case with highly disparsed eigenvalues
##    Error bound available
A <- diag((1:nv) ^ 2)
B <- diag((nv:1) ^ 3)
...
plot(res) # Clear evidence of non-convergence
...
#####
## VI: Difficult case with highly disparsed eigenvalues
##     Error bound unavailable

## check_convergence = "strict_relative" is recommended
## when error bound is unavailable
## (although this might be too strict)
...
sets `tol_conv = .Machine$double.eps`, which is rather strict
