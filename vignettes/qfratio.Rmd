...
The optional argument `simplify_ratio` is to specify the behavior in a special
condition when `A == B`.
When `TRUE` and `A == B`, the function tries to simplify the problem by
partially cancelling the exponents `p` and `q`, so that the new `p` is the
...
the series expression and calculation of error bound.
