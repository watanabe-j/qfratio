...
test_that("Expect identical results for simultaneously rotated matrices", {
    ...
    for(nv in nvs) {
        ...
        expect_equal(pqfr(qs, A1,  A2,  mu = mu,  method = "forchini", m_ser = 5, check_convergence = FALSE),
                     pqfr(qs, A1r, A2r, mu = mur, method = "forchini", m_ser = 5, check_convergence = FALSE), tolerance = tol)
        ...
        expect_equal(dqfr(qs, A1,                 method = "hillier", m_ser = 5, check_convergence = FALSE),
                     dqfr(qs, A1r,                method = "hillier", m_ser = 5, check_convergence = FALSE), tolerance = tol)
...
test_that("Expect equal from R and C++ methods", {
    ...
    for(nv in nvs) {
        ...
        expect_equal(pqfr(qs, A1, A2, mu = mu, method = "forchini", m_ser = 5, check_convergence = FALSE, use_cpp = TRUE),
                     pqfr(qs, A1, A2, mu = mu, method = "forchini", m_ser = 5, check_convergence = FALSE, use_cpp = FALSE), tolerance = tol)
        ...
        expect_equal(dqfr(qs, A1,              method = "hillier", m_ser = 5, check_convergence = FALSE, use_cpp = TRUE),
                     dqfr(qs, A1,              method = "hillier", m_ser = 5, check_convergence = FALSE, use_cpp = FALSE), tolerance = tol)
...
test_that("Expect similar results between different methods", {
    ...
    for(nv in nvs) {
        ...
        pseq_forchini <- pqfr(qs, A1, A2, mu = mu, method = "forchini", m_ser = 50, check_convergence = FALSE)
