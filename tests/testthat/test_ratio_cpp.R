    ...
    for(nv in nvs) {
        ...
        for(p in ks) {
            for(q in ks) {
                ...
                expect_equal(suppressWarnings(qfrm(A1, I,  p, q, m = m, mu = mu, check_convergence = FALSE, use_cpp = FALSE)),
                             suppressWarnings(qfrm(A1, I,  p, q, m = m, mu = mu, check_convergence = FALSE, use_cpp = TRUE)), tolerance = tol)
